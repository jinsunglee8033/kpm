<form method="GET" action="{{ route('copy.copy_jira') }}">
    <div class="form-row" style="background-color: white; margin: -16px 0px 0px 0px; padding: 0px 0px 0px 12px;">
        ...
        <div class="form-group col-md-2">
            <select class="design-select" name="copywriter">
                <option value="">Select Copywriter</option>
                @foreach ($copywriters as $assignee)
                    <option value="{{ $assignee['id'] }}" @if( $assignee['id'] == $copywriter) selected="selected" @endif >
                        {{$assignee['first_name']}}
                    </option>
                @endforeach
            </select>
