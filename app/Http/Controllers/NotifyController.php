...
use App\Mail\CopyTicketAssignToDo;
use App\Mail\CopyTicketRequest;
use App\Mail\CopyTicketReview;
...
//                    'url'           => '/admin/asset/'.$a_id.'/'.$c_id.'/'.$asset_type.'/detail_copy',
    ...
    public function copy_review($c_id, $a_id)
    {
        ...
        if($asset_author_id) {
            ...
            if ($campaign_rs['author_team'] == 'Business Division') {
                $cc_list = array();
                $brand_id = $campaign_rs['campaign_brand'];
                $brand_obj = new CampaignBrandsRepository();
                $brand_rs = $brand_obj->findById($brand_id);
...
//            Mail::to($name['email'])->send(new Todo($details));
    ...
    public function web_to_do($c_id, $a_id, $assignee)
    {
        $asset_index_obj = new CampaignAssetIndexRepository();
        $asset_index_rs = $asset_index_obj->findById($a_id);
        ...
        $user_obj = new UserRepository();
        $names = $user_obj->getEmailByWebName($assignee);
...
//            Mail::to($name['email'])->send(new Todo($details));
            Mail::to($name['email'])->send(new WebAssignToDo($details));
        }
    }
...
//            Mail::to($name['email'])->send(new Todo($details));
    ...
    public function copyticket_request($copy)
    {
        $user_obj = new UserRepository();
        $requestor = $user_obj->findById($copy->request_by);

        $rs = $user_obj->getCopyWriterManager();
        if($rs) {
            $cc_list = array();
            foreach ($rs as $copy_manager){
                $cc_list[] = $copy_manager['email'];
            }
            $details = [
                'who'           => $requestor['first_name'],
                'c_id'          => $copy->id,
                'task_name'     => $copy->title,
                'task_type'     => $copy->priority,
                'task_status'   => $copy->status,
                'url'           => '/admin/copy/'.$copy->id.'/edit',
            ];
            Mail::to($requestor['email'])
                ->cc($cc_list)
                ->send(new CopyTicketRequest($details));

        }
    }

    public function copyticket_to_do($c_id, $assign_to)
    {
        $copy_repo = new CopyRepository();
        $copy_array = $copy_repo->findById($c_id);
        ...
        $details = [
            ...
            'task_type'     => $copy_array['type'],
            'c_id'          => $c_id,
            'title'         => $copy_array['title'],
            'task_status'   => $copy_array['status'],
            'url'           => '/admin/copy/'.$c_id.'/edit',
        ];
        Mail::to($user_array['email'])->send(new CopyTicketAssignToDo($details));
    }

    public function copyticket_review($c_id, $request_by)
    {
        $copy_repo = new CopyRepository();
        $copy_array = $copy_repo->findById($c_id);

        $copy_repo = new UserRepository();
        $user_array = $copy_repo->findById($request_by);
        ...
        Mail::to($user_array['email'])->send(new CopyTicketReview($details));
    }
...
/************ */
    ...
    public function decline_from_kec($c_id, $a_id, $params)
    {
        ...
        $team_to = $asset_index_rs['team_to'];

        $user_obj = new UserRepository();
        if($team_to == 'web production'){
            $names = $user_obj->getEmailByWebName($asset_assignee);
        }else if($team_to == 'content'){
            $names = $user_obj->getEmailByContentName($asset_assignee);
        }else {
...
//                    Mail::to('jilee2@kissusa.com')->send(new ReminderDueBefore($details));
                    ...
                    if (isset($item->asset_author_id)) {
                        ...
                        if ($item->author_team == 'Business Division') {
